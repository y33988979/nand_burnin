...
int phy_expanded_read(struct eth_device *dev, u32 reg_addr)
{
	...
	miiphy_write (dev->name, UD_REG_NAME(PHY_ADDR), MII_EXPMA, 
		      (unsigned short)reg_addr);
	miiphy_read(dev->name, UD_REG_NAME(PHY_ADDR), MII_EXPMD, 
		    (unsigned short *)&ret);
...
int phy_expanded_write(struct eth_device *dev, u32 reg_addr, u16 val)
{
	...
	miiphy_write (dev->name, UD_REG_NAME(PHY_ADDR), MII_EXPMA, 
		      (unsigned short)reg_addr);
	ret = miiphy_write(dev->name, UD_REG_NAME(PHY_ADDR), MII_EXPMD, val);
			    ...
			    int count)
{
	...
	miiphy_read(dev->name, UD_REG_NAME(PHY_ADDR), PHY_BMCR, &v);
	v |= PHY_BMCR_POWD;
	miiphy_write (dev->name, UD_REG_NAME(PHY_ADDR), PHY_BMCR, v);
	for (i = 0; i < (2 * count); i += 2) {	
		reg_addr = reg_and_val[i];
		val = (u16) reg_and_val[i + 1];
		miiphy_write (dev->name, UD_REG_NAME(PHY_ADDR), 
			      MII_EXPMA, (unsigned short)reg_addr);
		ret = miiphy_write(dev->name, UD_REG_NAME(PHY_ADDR), 
				   MII_EXPMD, val);
	}
