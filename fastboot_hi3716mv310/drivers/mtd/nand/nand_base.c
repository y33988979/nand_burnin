			   ...
			   const uint8_t *buf, int page, int cached, int raw)
{
	...
	if (!cached || !(chip->options & NAND_CACHEPRG)) {
		 ...
		 * available
		 */
		if ((status & NAND_STATUS_PFAIL) && (chip->errstat))
			status = chip->errstat(mtd, chip, FL_WRITING, status,
					       page);

		if (status & NAND_STATUS_PFAIL)
			return -EIO;
	} else {
		    ...
		    int allowbbt)
{
	...
	while (len) {
		 ...
		 * available
		 */
		if ((status & NAND_STATUS_EFAIL) && (chip->errstat))
			status = chip->errstat(mtd, chip, FL_ERASING,
					       status, page);

		/* See if block erase succeeded */
		if (status & NAND_STATUS_EFAIL) {
						  ...
						  int busw, int *maf_id)
{
	...
	*maf_id = chip->read_byte(mtd);
	 ...
	 */
find_type:
	if (nand_param_adjust
	    && nand_param_adjust(mtd, chip, &nand_dev))
		return ERR_PTR(-ENODEV);
