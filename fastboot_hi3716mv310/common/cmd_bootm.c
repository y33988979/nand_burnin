		...
		bootm_headers_t *images, ulong *os_data, ulong *os_len)
{
	...
	if (!check_bootimg((char *)img_addr, MAX_BOOTIMG_LEN)) {
		...
		int result = bootimg_get_kernel(buf, MAX_BOOTIMG_LEN, (unsigned int*)&img_addr);
		debug("_________%d_______\n", result);
		if(result)
		{
			printf("%d\n", result);
...
#endif
	default:
		printf ("111111Wrong Image Format for %s command\n", cmdtp->name);
		show_boot_progress (-108);
		return NULL;
	}
